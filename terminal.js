path = {
    ...
    {
        'professeurludo': {
            'introduction.txt': 'L\'axe coding et digital innovation vous permettra d\'aquérir des compétences en développement web mais aussi en transformation digital.',
            'projets' : {
                'ecommerce.txt' : 'il existe de nombreux frameworks et cms qui vous permettront de créer des sites e-commerce plus facilement',
                'notions' : {
                    'framework.txt' : 'Un Framework est une boite à outils pour un développeur web. Frame signifie cadre et work se traduit par travail. Un Framework contient des composants autonomes qui permettent de faciliter le développement d’un site web ou d’une application. Ces composants résolvent des problèmes souvent rencontrés par les développeurs (CRUD, arborescence, normes, sécurités, etc.). Ils permettent donc de gagner du temps lors du développement du site.',
                    'CMS.txt' : 'CMS est l\'acronyme de Content Management System, c\'est-à-dire système de gestion de contenu. Il s\'agit d\'un logiciel en ligne grâce auquel il est possible de créer, de gérer et de modifier facilement un site web, sans avoir besoin de connaissances techniques en langage informatique.',
                }
            },
            'languages' : {
        ...
        'docteurmar' : {
            'informations.txt' : 'Dans l\'axe communication digital, vous développerez des compétences qui vous permettront de réaliser des études marketings et de mettre en place des stratégies digitales',
            'notions' : {
        ...
        'professeurAI': {
            ...
            'jeuxvideos' : {
                ...
                'gameart.docx' : '<p>Dessin traditionnel et anatomique, bases de la 2D, Painting / Texturing, Perspective / Architecture, Modélisation / Animation 3D, UI, FX, 3DS Max, Zbrush, Spine, Photoshop, Substance, Popcorn.</p>',
                'gameprogramming.docx' : '<p>Unity, Unreal, Programmation Orientée Objet (POO), C++, C#, Programmation 2D, Algorithmie, Design patterns.</p>',
        ...
        'docteurval' : {
            'important': {
                '3D.txt' : 'Dans l\'axe 3D, vous apprendrez à modéliser tous types d\'objets et personnages en 3D, vous apprendrez à vous servir des différents logiciels et outils de 3D.',
                'repotologie.docx' : '<p>Le principe de la retopologie, c\'est de créer un modèle par dessus un autre pour en faire une version adaptée et efficace. Pour de l\'animation par exemple. Quand on modélise un personnage, il est très utile de faire une version sculptée avant de travailler le maillage.</p>',
...
$('body').terminal({
    ...
    ls: function() {
        ...
        pathToObj = path
        for (let file in splitPath)
        {
            pathToObj = pathToObj[splitPath[file]];
        }
        this.echo(Object.keys(pathToObj));
    },
    cd: function(folder) {
        ...
        for(let currentPath in splitPath)
        {
            if (splitFolder.includes(".."))
            {
                if (Object.keys(pathToObj).includes(splitPath[splitPath.length - 1]))
                {
                    break;
                }
            }
            pathToObj = pathToObj[splitPath[currentPath]];
        }
        folder = splitFolder[splitFolder.length - 1]
        if (!splitFolder.includes(".."))
        {
        for(let file in splitFolder)
        {
            if(!Object.keys(pathToObj).includes(splitFolder[file]) || splitFolder[file].includes(".txt"))
            {
                return 'Folder does not exist';
            }

            pathToObj = pathToObj[splitFolder[file]];
        ...
        if (splitFolder.includes(".."))
        {
            newPath = splitPath
            newPath.pop()
            currentFile = newPath.join("/")
        }
        else
        {
            currentFile += '/' + splitFolder.join("/")
        }
...
help: function(){
        this.echo("cd <chemin du dossier> - Se déplacer vers le dossier correspondant ('cd ..' permet de revenir au dossier précédent) \nls - Afficher la liste des dossiers et fichiers présents dans le dossier actuel \ncat <nom du fichier> - Afficher le contenu d'un fichier \nPress CTRL+L to clear the terminal.")
    },
    cat: function(file) {
        currentPath = currentFile.split("/");
        pathToFile = path
        for(let directory in currentPath)
        {
            pathToFile = pathToFile[currentPath[directory]];
        }
        if (Object.keys(pathToFile).includes(file))
        {
            if(typeof pathToFile[file] == 'string')
            {
                if (file.includes(".docx"))
                {
                    this.echo($(pathToFile[file]))
                }
                else
                {
                    this.echo(pathToFile[file])
